...
8. Push your generic image (the one with the root user) to some registry if not done already.
   This will be handy for you, for sharing it with your teammates, and when you open-source your project later.
    ...
    3. Add this link to the TODO ADD PULL_IMAGE_NAME in
       ...
       e.g., ic-registry.epfl.ch/your-lab/your-gaspar/deep-alignment_.)
...
```bash
git clone <HTTPS/SSH> deep-alignment
cd deep-alignment
```
...
./template.sh run python -m deep_alignment.template_experiment some_arg=some_value
...
```bash
mv deep-alignment deep-alignment-tmp
mkdir deep-alignment
mv deep-alignment-tmp deep-alignment/dev
...
E.g., you can mount it at `/project/deep-alignment` and specify `PROJECT_ROOT_AT=/project/deep-alignment`.
...
export PROJECT_ROOT_AT=/project/deep-alignment
apptainer run \
    -c \
    ...
    --env WANDB_API_KEY="" \
    --nv deep-alignment_amd64-cuda-root-latest.sif
